...
#
# Writes: dist/Ace-Combat-3-Electrosphere-Japan-Recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix Ace-Combat-3-Electrosphere-Japan-Recomp \
  --exe-name Ace_Combat_3__Electrosphere__Japan__Recompiled \
  --display-name "Ace Combat 3: Electrosphere (Japan) Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Ace Combat 3: Electrosphere (Japan) disc" \
